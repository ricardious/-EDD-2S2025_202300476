...
uses
  SysUtils, User, SinglyLinkedList, DoublyLinkedList, Stack, Queue,
  CircularLinkedList, DotUtils;
...
uses AVLTree, BTree, EmailService;
...
begin
  ...
  NewU^.Drafts := TAVLTree.Create(@CompareEmailsByID);
  NewU^.Favorites := TBTree.Create(@CompareEmailsByID);
...
begin
  ...
  Result := Format('Id: %s' + '\n' + 'Name: %s' + '\n' + 'Username: %s' +
    '\n' + 'Email: %s' + '\n' + 'Phone: %s', [DotEscape(IntToStr(U^.Id)),
    DotEscape(U^.Name), DotEscape(U^.Username), DotEscape(U^.Email),
    DotEscape(U^.Phone)]);
end;
