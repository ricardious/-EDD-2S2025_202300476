...
function RemoveContactFromUser(var CurrentUser: TUser; const Email: string): integer;
...
function FindContactByUsername(var L: TCircularLinkedList;
  const Username: string): PCircularNode;
var
  Current: PCircularNode;
  User: PUser;
begin
...
begin
  ...
  if SameText(NormalizedEmail, CurrentUser.Username) then
    Exit(-3);

  Node := FindContactByUsername(CurrentUser.Contacts, NormalizedEmail);
  if Node = nil then Exit(-2);

  DeleteNode(CurrentUser.Contacts, Node);
  Result := 0;
end;
