    ...
    begin
      ...
      begin
        if Current = Q.Front then
          WriteLn(F, Format('    node%d [label="<data>%s|<next>", fillcolor=lightgreen];',
                   [NodeIndex, DataToString(Current^.Data)]))
        else if Current = Q.Rear then
          WriteLn(F, Format('    node%d [label="<data>%s|<next>", fillcolor=lightcoral];',
                   [NodeIndex, DataToString(Current^.Data)]))
        else
      ...
      WriteLn(F, Format('    rear -> node%d:data [color=darkred, style=bold, label="enqueue"];',
               [Count(Q) - 1]));

      WriteLn(F, '');

      WriteLn(F, Format('    info [label="Tamaño: %d\\nFIFO: First In, First Out", shape=note, fillcolor=lightyellow];',
               [Count(Q)]));
